...
main () {
  ...
  group("HandlingEvent", () {
    ...
    setUp(() {
      ...
      RouteSpecification routeSpec = new RouteSpecification(HONGKONG, NEWYORK, new Date.now());
      cargo = new Cargo(trackingId, routeSpec);
    });
    
    test("new with carrier movement", () {
      HandlingEvent e1 = new HandlingEvent(cargo, new Date.now(), new Date.now(), HandlingEventType.LOAD, HONGKONG, CM003);
      expect(e1.location, equals(HONGKONG));
      
      HandlingEvent e2 = new HandlingEvent(cargo, new Date.now(), new Date.now(), HandlingEventType.UNLOAD, NEWYORK, CM003);
      ...
      for (var type in [HandlingEventType.CLAIM, HandlingEventType.RECEIVE, HandlingEventType.CUSTOMS]) {
        expect(
            () => new HandlingEvent(cargo, new Date.now(), new Date.now(), type, HONGKONG, CM003),
      ...
      for (var type in [HandlingEventType.LOAD, HandlingEventType.UNLOAD]) {
        expect(
            () => new HandlingEvent(cargo, new Date.now(), new Date.now(), type, HONGKONG, null),
            throwsA(new isInstanceOf<ExpectException>()));
      }
    });
    
    test("new with location", () {
      HandlingEvent e1 = new HandlingEvent(cargo, new Date.now(), new Date.now(), HandlingEventType.CLAIM, HELSINKI);
      expect(e1.location, equals(HELSINKI));
    });
    
    test("current location load event", () {
      HandlingEvent ev = new HandlingEvent(cargo, new Date.now(), new Date.now(), HandlingEventType.LOAD, CHICAGO, CM004);
      expect(ev.location, equals(CHICAGO));
    });
    
    test("current location unload event", () {
      HandlingEvent ev = new HandlingEvent(cargo, new Date.now(), new Date.now(), HandlingEventType.UNLOAD, HAMBURG, CM004);
      expect(ev.location, equals(HAMBURG));
    });
    
    test("current location received event", () {
      HandlingEvent ev = new HandlingEvent(cargo, new Date.now(), new Date.now(), HandlingEventType.RECEIVE, CHICAGO);
      expect(ev.location, equals(CHICAGO));
    });
    
    test("current location claimed event", () {
      HandlingEvent ev = new HandlingEvent(cargo, new Date.now(), new Date.now(), HandlingEventType.CLAIM, CHICAGO);
      expect(ev.location, equals(CHICAGO));
    });
    
    test("equals and samve as", () {
      Date timeOccured = new Date.now();
      Date timeRegistered = new Date.now();
  ...
  group("HandlingEventFactory", () {
    ...
    setUp(() {
      ...
      RouteSpecification routeSpec = new RouteSpecification(TOKYO, HELSINKI, new Date.now());
      cargo = new Cargo(trackingId, routeSpec);
    });
    ...
    test("create handling event with carrier movement", () {
      ...
      var handlingEvent = factory.createHandlingEvent(
        new Date.now(), date(100), trackingId, voyageNumber, unLocode, HandlingEventType.LOAD);
      ...
      expect(handlingEvent.registrationTime < new Date.now().add(new Duration(milliseconds:1)), isTrue);
    });
    ...
    test("create handling event without carrier movement", () {
      ...
      var unLocode = STOCKHOLM.unLocode;
      var handlingEvent = factory.createHandlingEvent(
        new Date.now(), date(100), trackingId, null, unLocode, HandlingEventType.CLAIM);
      ...
      expect(
        () {
          ...
          factory.createHandlingEvent(
              new Date.now(), date(100), trackingId, CM001.voyageNumber, invalid, HandlingEventType.LOAD);
        },
    ...
    test("create handling event unknown carrier movement", () {
      ...
      expect(
        () {
          ...
          factory.createHandlingEvent(
              new Date.now(), date(100), trackingId, invalid, STOCKHOLM.unLocode, HandlingEventType.LOAD);
        },
    ...
    test("create handling event unknown tracking id", () {
      ...
      expect(
        () {
          factory.createHandlingEvent(
              new Date.now(), date(100), trackingId, CM001.voyageNumber, STOCKHOLM.unLocode, HandlingEventType.LOAD);
        },
  ...
  group("HandlingHistory", (){
    ...
    setUp(() {
      cargo = new Cargo(new TrackingId("ABC"), new RouteSpecification(SHANGHAI, DALLAS, new Date(2009, 4, 1)));
      voyage = 
          (new VoyageBuilder(new VoyageNumber("X25"), HONGKONG)
            ..addMovement(SHANGHAI, new Date.now(), new Date.now())
            ..addMovement(DALLAS, new Date.now(), new Date.now())).build();
      
      event1 = new HandlingEvent(cargo, new Date(2009, 3, 5), date(100), HandlingEventType.LOAD, SHANGHAI, voyage);
      event1duplicate = new HandlingEvent(cargo, new Date(2009, 3, 5), date(200), HandlingEventType.LOAD, SHANGHAI, voyage);
      event2 = new HandlingEvent(cargo, new Date(2009, 3, 10), date(150), HandlingEventType.UNLOAD, DALLAS, voyage);
